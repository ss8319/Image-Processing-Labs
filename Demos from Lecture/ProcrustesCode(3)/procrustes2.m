...
% But also includes estimating scale change.
% Created 2008
% AAB
close all;
...
NewCoords = ScaleFactor*A*OriginalCoords + [Tx Tx Tx Tx Tx Tx Tx;Ty Ty Ty Ty Ty Ty Ty];% Rotate and translate
...
% This is an extra bit that I have included which seems to work rather
% better than the GPA solution. Not sure why !
SF_est=sqrt(mean(XAdash.^2)/mean(XBdash.^2));  % Unstable if you have too few points, of course.
...
%This tries to map A to B
XAtest = SF_est*R_est*(XB-muB*ones(1,7))+muA*ones(1,7);
xAtest = [XAtest(1,:)];
yAtest = [XAtest(2,:)];

%This tries to map B to A
XBtest = inv(SF_est*R_est)*(XA-muA*ones(1,7))+muB*ones(1,7);
xBtest = [XBtest(1,:)];
yBtest = [XBtest(2,:)];
...
h=line(xBtest,yBtest);axis([-20 60 -20 60]);set(h,'LineWidth',2);set(h,'Color','m');
text(xBtest(1:end-1),yBtest(1:end-1),{'1','2','3','4','5','6'});
